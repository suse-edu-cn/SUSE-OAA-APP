...
class AnnouncementRepositoryImpl(
    ...
    override val departments = listOf("协会", "算法竞赛部", "项目实践部", "组织宣传部", "秘书处")

    override suspend fun fetchAnnouncementInfo(department: String): Result<AnnouncementData> {
        return try {
            val response = api.getAnnouncementInfo(department)
            if (response.code == 200 && response.data != null) {
                Result.success(response.data)
            } else {
                appFailure(AppError.Business(userMessage = response.message.ifEmpty { "获取公告失败" }))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    override suspend fun updateAnnouncementInfo(department: String, content: String): Result<String> {
        return try {
            ...
            if (response.code == 200) {
                ...
                appFailure(AppError.Business(userMessage = response.message.ifEmpty { "更新公告失败" }))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
