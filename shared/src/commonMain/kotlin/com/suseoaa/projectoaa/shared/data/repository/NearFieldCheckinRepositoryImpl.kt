 ...
 */
class NearFieldCheckinRepositoryImpl(
    private val database: CourseDatabase,
    private val json: Json
) : NearFieldCheckinRepository {
     ...
     * 保存或更新签到任务到本地数据库
     */
    override suspend fun saveTask(task: NearFieldCheckinTask, isMyHosted: Boolean) {
     ...
     * 获取所有本地保存的签到任务历史
     */
    override fun getTaskHistory(): Flow<List<NearFieldTask>> {
     ...
     * 获取指定任务的所有签到人员
     */
    override fun getParticipants(taskIdentifier: String): Flow<List<NearFieldParticipant>> {
     ...
     * 删除指定的签到任务及其所有参与者记录
     */
    override suspend fun deleteTask(taskIdentifier: String) {
        withContext(Dispatchers.Default) {
     ...
     * 手动添加签到人员（用于补签或后台录入）
     */
    override suspend fun manualAddParticipant(
     ...
     */
    override fun startCheckinServer(port: Int, taskIdentifier: String) {
        serverJob?.cancel()
     ...
     * 停止签到服务器
     */
    override fun stopCheckinServer() {
        serverJob?.cancel()
        serverJob = null
    }
    ...
    override suspend fun sendCheckinRequest(
        ...
        studentName: String,
        studentId: String,
        status: String): Result<Unit> {
        val host = task.hostAddress ?: return appFailure(AppError.Business(userMessage = "无法获取主机地址"))
        ...
        return withContext(Dispatchers.Default) {
            ...
            try {
                ...
                var resultToReturn: Result<Unit> = appFailure(AppError.Business(userMessage = "未收到主机的响应"))
                if (responseLine != null) {
                    ...
                    if (response.success) {
                        ...
                        resultToReturn =
                            appFailure(AppError.Business(userMessage = response.errorMessage ?: "签到被拒绝"))
                    }
                }
     ...
     */
    override suspend fun syncParticipants(task: NearFieldCheckinTask): Result<Unit> {
        val host = task.hostAddress ?: return appFailure(AppError.Business(userMessage = "无法获取主机地址"))
        val port = task.hostPort ?: 8888
