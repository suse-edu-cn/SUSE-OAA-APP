 ...
 */
class AcademicStatusRepositoryImpl(
     ...
     */
    override suspend fun getAcademicStatusCategories(studentId: String): Result<Pair<AcademicPlanOverview, List<AcademicStatusCategory>>> =
        withContext(Dispatchers.IO) {
            try {
                ...
                if (response.status.value == 200) {
                    ...
                    appFailure(AppError.Http(response.status.value, "获取学业情况失败: ${response.status.value}"))
                }
            } catch (e: Exception) {
                e.printStackTrace()
                Result.failure(e)
     ...
     * @param studentId 学号
     */
    override suspend fun getCategoryCourses(
        categoryId: String,
        studentId: String
        ...
        try {
            ...
            if (response.status.value == 200) {
                ...
                appFailure(AppError.Http(response.status.value, "获取课程列表失败: ${response.status.value}"))
            }
        } catch (e: Exception) {
            e.printStackTrace()
            Result.failure(e)
    ...
    override suspend fun getOtherCourses(
        ...
        try {
            ...
            if (response.status.value == 200) {
                ...
                appFailure(AppError.Http(response.status.value, "获取其它课程列表失败: ${response.status.value}"))
            }
        } catch (e: Exception) {
            e.printStackTrace()
            Result.failure(e)
     ...
     * 获取非计划内课程
     */
    override suspend fun getNonPlanCourses(
        categoryId: String,
        studentId: String
    ): Result<List<AcademicStatusCourseItem>> = withContext(Dispatchers.IO) {
        try {
            ...
            if (response.status.value == 200) {
                ...
                appFailure(AppError.Http(response.status.value, "获取非计划课程失败: ${response.status.value}"))
            }
        } catch (e: Exception) {
            e.printStackTrace()
            Result.failure(e)
     ...
     */
    override fun calculateCategoryStats(courses: List<AcademicStatusCourseItem>): AcademicStatusCategory {
     ...
     * 不及格课程 JD=0，会拉低平均绩点
     */
    override fun calculateWeightedGpa(allCourses: List<AcademicStatusCourseItem>): Double {
