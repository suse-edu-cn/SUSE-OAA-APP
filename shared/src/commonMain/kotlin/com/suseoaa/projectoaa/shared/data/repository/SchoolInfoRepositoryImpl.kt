...
class SchoolInfoRepositoryImpl(
     ...
     * 观察考试信息流
     */
    override fun observeExams(studentId: String): Flow<List<ExamCacheEntity>> {
     ...
     * 刷新考试信息
     */
    override suspend fun refreshAcademicExamInfo(account: CourseAccountEntity): Result<String> =
        withContext(Dispatchers.IO) {
            try {
                ...
                if (response.status.value == 200) {
                    ...
                    appFailure(AppError.Http(response.status.value, "获取考试信息失败: ${response.status.value}"))
                }
            } catch (e: Exception) {
                e.printStackTrace()
                Result.failure(e)
     ...
     * 根据学期获取考试信息（不缓存，直接返回）
     */
    override suspend fun fetchExamsByTerm(
     ...
     * 添加自定义考试
     */
    override suspend fun addCustomExam(exam: ExamCacheEntity): Unit = withContext(Dispatchers.IO) {
        database.examCacheQueries.insert(
            studentId = exam.studentId,
            courseName = exam.courseName,
            time = exam.time,
     ...
     * 更新考试信息
     */
    override suspend fun updateExam(exam: ExamCacheEntity): Unit = withContext(Dispatchers.IO) {
     ...
     * 删除考试信息
     */
    override suspend fun deleteExam(examId: Long): Unit = withContext(Dispatchers.IO) {
     ...
     * 获取指定学期的自定义考试信息（同步方法）
     */
    override suspend fun getCustomExamsBySemester(
        studentId: String,
        xnm: String,
        xqm: String
     ...
     * 观察指定学期的考试信息
     */
    override fun observeExamsBySemester(
        studentId: String,
        xnm: String,
        xqm: String
     ...
     * 观察调课通知流
     */
    override fun observeMessages(studentId: String): Flow<List<MessageCacheEntity>> {
     ...
     * 刷新调课通知
     */
    override suspend fun refreshAcademicMessageInfo(account: CourseAccountEntity): Result<String> =
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (response.status.value == 901 || response.status.value == 302 || isLoginRequired(
                    ...
                    if (loginResult.isFailure) return@withContext appFailure(AppError.Business(userMessage = "自动登录失败"))
                ...
                if (response.status.value == 200) {
                    ...
                    appFailure(AppError.Http(response.status.value, "请求失败: ${response.status.value}"))
                }
            } catch (e: Exception) {
                Result.failure(e)
            }
     ...
     * 写回 AI 摘要
     */
    override suspend fun updateMessageSummary(id: Long, summary: String): Unit = withContext(Dispatchers.IO) {
     ...
     */
    override suspend fun getAcademicCourseInfo(account: CourseAccountEntity): Result<List<String>> =
