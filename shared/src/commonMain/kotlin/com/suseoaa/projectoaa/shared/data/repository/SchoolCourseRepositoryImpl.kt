...
class SchoolCourseRepositoryImpl(
    private val api: SchoolApiService,
    private val json: Json
) : SchoolCourseRepository {
    override suspend fun getCourseSchedule(year: String, semester: String): Result<CourseResponseJson> {
        return try {
            val response = api.querySchedule(year = year, semester = semester)
            AppLog.d("[SchoolCourseRepository] 课表查询返回状态码: ${response.status.value}")
            AppLog.d("[SchoolCourseRepository] 响应头信息: ${response.headers.entries()}")

            if (response.status.value == 200) {
                val jsonString = response.bodyAsText()

                if (isLoginRequired(jsonString)) {
                    return appFailure(AppError.SessionExpired)
                }
                ...
                try {
                    ...
                    appFailure(AppError.Parse("JSON 解析失败: ${e.message}"))
                }
            } else {
                if (response.status.value == 302 || response.status.value == 901) {
                    appFailure(AppError.SessionExpired)
                } else {
     ...
     */
    override suspend fun fetchSemesterStart(): SemesterCalendarInfo? {
        return try {
