...
class CheckinRepositoryImpl(
    ...
    override fun getAllAccounts(): List<CheckinAccountData> = accountStore.getAll()

    override fun getAccountById(id: Long): CheckinAccountData? = accountStore.getById(id)
    ...
    override fun deleteAccount(id: Long): Result<Unit> = accountStore.delete(id)

    override fun isAccountExists(studentId: String): Boolean = accountStore.exists(studentId)
    ...
    override fun clearSession(accountId: Long): Result<Unit> = accountStore.clearSession(accountId)

    override fun updateLocation(accountId: Long, locationName: String): Result<Unit> =
        accountStore.updateLocation(accountId, locationName)

    // ==================== 登录 ====================

    override suspend fun tryAutoLoginWithRememberMe(account: CheckinAccountData): Result<Boolean> =
        loginRepository.tryAutoLoginWithRememberMe(account)

    override suspend fun fetchCaptchaImage(): Result<ByteArray> = loginRepository.fetchCaptchaImage()
    ...
    override fun isSmsVerificationRequired(error: Throwable?): Boolean =
        loginRepository.isSmsVerificationRequired(error)

    override fun hasPendingSmsChallenge(): Boolean = loginRepository.hasPendingSmsChallenge()

    override fun getPendingSmsMaskedPhone(): String? = loginRepository.getPendingSmsMaskedPhone()

    override fun clearPendingSmsChallenge() = loginRepository.clearPendingSmsChallenge()

    override suspend fun sendSmsCodeForPendingLogin(): Result<Unit> =
        loginRepository.sendSmsCodeForPendingLogin()

    override suspend fun submitSmsCodeForPendingLogin(smsCode: String): Result<Unit> =
    ...
    override suspend fun checkinForSpecificTaskInternal(
        taskId: Long,
