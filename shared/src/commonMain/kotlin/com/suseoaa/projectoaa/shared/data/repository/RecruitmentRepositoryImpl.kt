...
class RecruitmentRepositoryImpl(private val apiService: RecruitmentApiService) : RecruitmentRepository {

    override suspend fun createApplication(application: RecruitmentApplication): Result<RecruitmentResponse<Unit>> {
        return try {
            val response = apiService.createApplication(application.toSubmitRequest())
            if (response.code == 200) {
                Result.success(response)
            } else {
                appFailure(AppError.Business(userMessage = response.message.ifEmpty { "提交申请失败" }))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    override suspend fun getApplications(): Result<RecruitmentResponse<List<RecruitmentApplication>>> {
        return try {
            val response = apiService.getApplications()
            if (response.code == 200) {
                Result.success(response)
            } else {
                appFailure(AppError.Business(userMessage = response.message.ifEmpty { "获取申请失败" }))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    override suspend fun updateApplication(application: RecruitmentApplication): Result<RecruitmentResponse<RecruitmentApplication>> {
        return try {
            val response = apiService.updateApplication(application.toSubmitRequest())
            if (response.code == 200) {
                Result.success(response)
            } else {
                appFailure(AppError.Business(userMessage = response.message.ifEmpty { "更新失败" }))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    override suspend fun uploadImage(imageBytes: ByteArray, filename: String): Result<String> {
        return try {
            ...
            if (response.code == 200) {
                ...
                appFailure(AppError.Business(userMessage = response.message.ifEmpty { "上传失败" }))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    override suspend fun updateTime(request: ChangeTimeRequest): Result<String> {
        return try {
            ...
            if (response.code == 200) {
                ...
                appFailure(AppError.Business(userMessage = response.message.ifEmpty { "修改时间失败" }))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    override suspend fun changeStatus(request: ChangeStatusRequest): Result<String> {
        return try {
            ...
            if (normalized == "更新成功") {
                ...
                appFailure(AppError.Business(userMessage = extractErrorMessage(normalized)))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
