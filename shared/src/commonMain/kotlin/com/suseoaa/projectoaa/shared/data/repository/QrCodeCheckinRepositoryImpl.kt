 ...
 */
class QrCodeCheckinRepositoryImpl(
    ...
    /** 步骤1: 获取微信扫码 ClientId */
    override suspend fun getClientId(): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            AppLog.d("[QrCheckin] 获取 ClientId...")
            ...
            AppLog.d("[QrCheckin] ClientId: $clientId")
            clientId
        }.onFailure { AppLog.e("[QrCheckin] 获取 ClientId 异常: ${it.message}") }
    }

    /** 步骤2: 获取微信二维码图片（Base64） */
    override suspend fun getQrCodeImage(clientId: String): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            AppLog.d("[QrCheckin] 获取二维码, clientId=$clientId")
            ...
            AppLog.d("[QrCheckin] 二维码图片长度: ${image.length}")
            image
        }.onFailure { AppLog.e("[QrCheckin] 获取二维码异常: ${it.message}") }
    }

    /** 步骤3: 轮询扫码状态。status: 0=等待扫码, 1=已扫码待确认, 2=已确认 */
    override suspend fun checkScanStatus(clientId: String): Result<WechatScanStatusData> =
        withContext(Dispatchers.IO) {
            runCatching {
                val response = api.checkScanStatus(clientId)
                if (response.status.value != 200) throw CheckinException("检查扫码状态失败")
                val responseText = response.bodyAsText()
                AppLog.d("[QrCheckin] 扫码状态: $responseText")
                json.decodeFromString<WechatScanStatusResponse>(responseText).data
                    ?: throw CheckinException("扫码状态数据为空")
            }.onFailure { AppLog.e("[QrCheckin] 检查扫码状态异常: ${it.message}") }
        }

    /** 步骤4: 处理扫码回调，拿到 `_sop_session_` Cookie */
    override suspend fun handleScanCallback(callbackUrl: String): Result<String> =
        withContext(Dispatchers.IO) {
            runCatching {
                AppLog.d("[QrCheckin] 处理回调: $callbackUrl")
                val response = api.handleCallback(callbackUrl)
                AppLog.d("[QrCheckin] 回调响应状态: ${response.status.value}")
                ...
                AppLog.d("[QrCheckin] _sop_session_ 长度: ${sopSession.length}")
                "$SOP_SESSION=$sopSession"
            }.onFailure { AppLog.e("[QrCheckin] 处理回调异常: ${it.message}") }
        }
     ...
     */
    override suspend fun getSessionCookie(sopSessionCookie: String): Result<String> =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                if (session.isNullOrBlank()) {
                    AppLog.d("[QrCheckin] SSO API 未返回 SESSION，Location=${ssoResponse.headers["Location"]}")
                    cookieStorage.debugPrintAllCookies()
                    throw CheckinException("SSO API 未返回 SESSION Cookie")
                }

                AppLog.d("[QrCheckin] 成功获取 SESSION: ${session.take(20)}...")
                "$sopSessionCookie; $SESSION=$session"
            }.onFailure { AppLog.e("[QrCheckin] 获取 SESSION 异常: ${it.message}") }
        }

    /** 从任意 Cookie 字符串出发完成 SSO，得到含 SESSION 的完整 Cookie */
    override suspend fun completeSsoWithSopSession(cookies: String): Result<String> =
        withContext(Dispatchers.IO) {
    ...
    override suspend fun getEduUserInfoWithCookies(cookies: String): Result<EduUserInfo> =
        withContext(Dispatchers.IO) {
            runCatching {
                AppLog.d("[QrCheckin] 使用 Cookie 获取用户信息...")
            ...
            }.onFailure { AppLog.e("[QrCheckin] 获取用户信息异常: ${it.message}") }
        }
     ...
     */
    override suspend fun refreshSessionIfExpired(account: CheckinAccountData): Result<String> {
        val cookies = account.sessionToken
            ?: return Result.failure(CheckinException("Session token is null"))
        AppLog.d("[QrCheckin] 正在自动刷新账号 ${account.studentId} 的 Session...")
        return completeSsoWithSopSession(cookies)
            .onSuccess { newCookies ->
                ...
                AppLog.d("[QrCheckin] 账号 ${account.studentId} Session 自动刷新成功")
            }
            .onFailure { AppLog.e("[QrCheckin] 账号 ${account.studentId} Session 刷新失败: ${it.message}") }
    }
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                var cookies: String = if (account.isSessionValid()) {
                    ...
                    AppLog.d("[QrCheckin] Session 已过期，尝试自动刷新...")
                ...
                if (pendingResult.isSessionExpired()) {
                    AppLog.d("[QrCheckin] 任务请求返回登录已失效，尝试刷新 Session...")
                ...
                AppLog.e("[QrCheckin] 签到异常: ${e.message}")
    ...
    override suspend fun getAllTasksWithCookies(
        cookies: String,
