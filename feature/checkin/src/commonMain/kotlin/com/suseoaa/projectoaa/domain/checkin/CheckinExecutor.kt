...
class CheckinExecutor(
    ...
    private suspend fun executeForAccount(
        ...
        repeat(maxRetry + 1) { attempt ->
            if (attempt > 0) {
                AppLog.d("[CheckinExecutor] 账号 ${account.studentId} 第 $attempt 次重试")
        ...
        return try {
            when (val loginResult = autoLogin.login(account)) {
                is AutoLoginResult.SmsRequired -> {
                    AppLog.d("[CheckinExecutor] 账号 ${account.studentId} 需要短信验证，跳过")
                    Pair(false, "需要短信验证")
                }

                is AutoLoginResult.Failed -> {
                    AppLog.e("[CheckinExecutor] 账号 ${account.studentId} 登录失败: ${loginResult.message}")
            ...
            AppLog.e("[CheckinExecutor] 账号 ${account.studentId} 异常: ${e.message}")
