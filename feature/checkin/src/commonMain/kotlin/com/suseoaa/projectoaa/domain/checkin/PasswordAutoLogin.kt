package com.suseoaa.projectoaa.domain.checkin
...
class PasswordAutoLogin(private val repository: CheckinRepository) {
    ...
    private suspend fun attempt(account: CheckinAccountData, retryCount: Int): AutoLoginResult {
        return try {
            ...
            if (retryCount == 0) {
                ...
                if (fastLogin) {
                    AppLog.d("[AutoLogin] 使用 rememberMe 快速登录成功")
                    return AutoLoginResult.Success
                }
                AppLog.d("[AutoLogin] rememberMe 快速登录未命中，回退验证码登录")
            }
            ...
            if (captchaResult.isFailure) {
                ...
                AppLog.e("[AutoLogin] 获取验证码失败: $message")
            ...
            AppLog.d("[AutoLogin] OCR识别成功: $captchaCode")
            ...
            AppLog.e("[AutoLogin] 登录失败: $message")
            AutoLoginResult.Failed(message)
        } catch (e: Throwable) {
            AppLog.e("[AutoLogin] 异常: ${e.message}")
    ...
    private suspend fun retryOrFail(
        ...
        AppLog.d("[AutoLogin] $message，重试第 ${retryCount + 1} 次")
